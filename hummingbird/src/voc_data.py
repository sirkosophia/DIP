# Code from: https://github.com/vpariza/open-hummingbird-eval
...
class VOCDataModule(pl.LightningDataModule):
    ...
    def __init__(self,
        ...
        self.voc_train = VOCDataset(root=self.root, image_set=train_split, transforms=self.train_image_transform,
                                    return_masks=self.return_masks, subsample_file=self.subsample_file)
...
class VOCDataset(VisionDataset):
    ...
    # either transform and target_transform should be passed or only transforms
        ...
        if not os.path.isdir(seg_dir) or not os.path.isdir(image_dir) or not os.path.isdir(root):
            print("seg_dir {0} ".format(seg_dir))
            print("image_dir {0}".format(image_dir))
            print("root {0}".format(root))
            raise RuntimeError('Dataset not found or corrupted.')
        if not os.path.isdir(seg_dir) or not os.path.isdir(image_dir) or not os.path.isdir(root):
            raise RuntimeError('Dataset not found or corrupted.')
        
        if subsample_file is not None and os.path.exists(subsample_file):
            print(f"Loading subsample from: {subsample_file}")
            with open(subsample_file, "r") as f:
                file_names = [x.strip() for x in f.readlines()]
        else:
            # Use the default dataset split
            splits_dir = os.path.join(root, 'sets')
            if not os.path.exists(splits_dir):
                folder_name = 'SegmentationAug' if (self.image_set == "trainaug" or self.image_set == "train") else 'Segmentation'
                splits_dir = os.path.join(root, 'ImageSets', folder_name)
